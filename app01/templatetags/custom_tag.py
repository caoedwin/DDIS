...
@register.filter
def get_rbac_menu_items(request):
    """将菜单数据转换为适合顶部菜单的结构"""
    # 获取原始菜单数据（假设你已经有获取菜单数据的函数）
    raw_menu_data = get_structure_data(request)  # 替换为实际获取菜单数据的函数

    # 转换数据结构
    top_menu_items = []
    for item in raw_menu_data:
        if not item['status']:  # 跳过无权限的菜单项
            continue

        # 创建顶部菜单项
        menu_item = {
            'title': item['title'].split("_")[-1],
            'icon': get_icon_class(item['title']),  # 获取图标类名
            'children': []
        }
        ...
        if item.get('children'):
            for child in item['children']:
                if not child.get('status', True):  # 跳过无权限的子菜单项
                    continue
                ...
                if child.get('children'):
                    # 创建中间节点菜单项
                    child_item = {
                        'title': child['title'].split("_")[-1],
                        'icon': get_icon_class(child['title']),
                        'children': []
                    }

                    # 处理子子菜单
                    for sub_child in child['children']:
                        if not sub_child.get('status', True):  # 跳过无权限的子菜单项
                            continue

                        # 处理叶子节点（有URL的节点）
                        if sub_child.get('url'):
                            child_item['children'].append({
                                'title': sub_child['title'].split("_")[-1],
                                'url': sub_child['url']
                            })
                        # 处理更深层次的中间节点
                        elif sub_child.get('children'):
                            # 递归处理更深层次的菜单
                            child_item['children'].extend(process_deeper_levels(sub_child))
...
def process_deeper_levels(node):
    """递归处理更深层次的菜单节点"""
    items = []
    for child in node.get('children', []):
        ...
        if child.get('url'):
            ...
            child_item = {
                'title': child['title'].split("_")[-1],
                'icon': get_icon_class(child['title']),
                'children': process_deeper_levels(child)
            }
...
def get_icon_class(title):
    """根据标题获取图标类名（从你的 rbac_menu 函数中提取）"""
    if title == 'Lesson Learn' or title.startswith('Lesson Learn-'):
        return "ti-file"
    elif title == 'SW&ME' or title.startswith('SW&ME-') or title == 'IssuesBreakdown':
        return "ti-agenda"
    elif title == 'Compatibility':
        return "ti-layout-grid4-alt"
    elif title == 'QIL' or title.startswith('QIL-') or title == 'Critical Issue':
        return "ti-layout-grid2-alt"
    elif title == 'Reliability Test Data':
        return "ti-archive"
    elif title == 'Package G Value':
        return "ti-layout"
    elif title == 'Bouncing' or title == 'ProjectComparison':
        return "ti-panel"
    elif title == 'CDM':
        return "ti-view-list-alt"
    elif title == 'XQM':
        return "ti-files"
    elif title == 'CQM' or title == '申購記錄':
        return "ti-target"
    elif title == 'MQM':
        return "ti-bar-chart-alt"
    elif 'DriverToolList' in title:
        return "ti-layout-cta-right"
    elif title == 'Others':
        return "ti-map"
    elif title == 'Known issue list':
        return "ti-layout-column3"
    elif title == 'Runin Report':
        return "ti-layout-list-thumb"
    elif title == 'Test Plan':
        return "ti-bar-chart-alt"
    elif title == 'ME':
        return "ti-layout-media-overlay-alt"
    elif title == 'SW' or title == 'SW-OR':
        return "ti-layout-width-default"
    elif title == 'INV':
        return "ti-layout-list-large-image"
    elif title == 'SpecDownload' or title == 'RD/PE/JQE-Lesson':
        return "ti-layout-accordion-list"
    elif title == 'Issue Notes' or title == '資本支出':
        return "ti-layout-column3"
    elif title == 'Issue List':
        return "ti-bookmark"
    elif title == 'Known Issue':
        return "ti-envelope"
    elif title == 'DepartmentManage':
        return "ti-cloud"
    elif title == 'PersonalInfo':
        return "ti-id-badge"
    elif title == '公共區域':
        return "ti-comment"
    elif title == 'ProjectInfo':
        return "ti-search"
    elif title == 'OBIDeviceResult':
        return "ti-light-bulb"
    elif title == 'Automation效益':
        return "ti-panel"
    elif title == 'ABO' or title == 'LowLight' or title == 'Input' or title == '我的' or title == 'Summary' or title == '人員測試履歷':
        return "ti-view-list-alt"
