...
@admin.register(files_SopRom)
@admin.register(files_SopProcess)
@admin.register(files_SopDevice)
...
@admin.register(ManagementSopRoom)
class ManagementSopRoomAdmin(admin.ModelAdmin):
    list_display = ('Num', 'Title', 'Description', "Version", "editor", "edit_time")
    # 列表里显示想要显示的字段
    list_per_page = 500
    # 满50条数据就自动分页
    ordering = ('-edit_time',)
    # 后台数据列表排序方式
    list_display_links = ('Num', 'Title', 'Description', "Version", "editor", "edit_time")
    # 设置哪些字段可以点击进入编辑界面
    # list_editable = ('Object',)
    # 筛选器
    # list_filter = ('Customer','Project', 'Unit', 'Phase', 'Tester', 'Testitem','Result', 'Start_time', 'End_time', 'Result_time','Item_Des', 'Comments')  # 过滤器
    list_filter = (
                    'editor',)  # 过滤器
    search_fields = ('editor',)  # 搜索字段
    # date_hierarchy = 'Start_time'  # 详细时间分层筛选
    filter_horizontal = ('files_Spec',)

@admin.register(ManagementSopProcess)
class ManagementSopProcessAdmin(admin.ModelAdmin):
    list_display = ('Num', 'Title', 'Description', "Version", "editor", "edit_time")
    # 列表里显示想要显示的字段
    list_per_page = 500
    # 满50条数据就自动分页
...
@admin.register(ManagementSopDevice)
class ManagementSopDeviceAdmin(admin.ModelAdmin):
    list_display = ('Num', 'Title', 'Description', "Version", "editor", "edit_time")
    # 列表里显示想要显示的字段
    list_per_page = 500
    # 满50条数据就自动分页
