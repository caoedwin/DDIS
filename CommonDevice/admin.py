...
class CommonDeviceAdmin(admin.ModelAdmin):
    fieldsets = (
        ...
        'Purchasing_Date', 'Department', 'Location', 'Asset_Num', 'Owner_Num', 'Owner', 'Mail', 'Contact_info', 'Comments', 'Editor',)
        }),
        # ('Advanced options',{
        #     'classes': ('collapse',),
    ...
    list_display = (
        ...
        'Purchasing_Date', 'Department', 'Location', 'Asset_Num', 'show_user', 'Owner', 'Mail', 'Contact_info', 'Comments', 'Editor', 'created_at', 'updated_at',
                    )
    def show_user(self, obj):
        user_list = []
        for user in obj.Owner_Num.all():
            user_list.append(user.account+'-'+user.username)
        return '， '.join(user_list)

    show_user.short_description = 'onwer_num'  # 设置表头
    filter_horizontal = ('Owner_Num',)
    # 列表里显示想要显示的字段
    list_per_page = 500
    # 满50条数据就自动分页
    ordering = ('-created_at',)
    ...
    list_display_links = ('Category', 'Product_Type', 'Name', 'Num', 'Manufacturer', 'Cost', 'Account', 'PW',
        'Purchasing_Date', 'Department', 'Location', 'Asset_Num', 'Owner', 'Mail', 'Contact_info',  'Comments' )
    # 设置哪些字段可以点击进入编辑界面
    ...
    search_fields = ('Owner_Num', 'Owner', 'Category', 'Product_Type', 'Owner_Num__account',)  # 搜索字段
