...
from .models import A31lesson_learn, A31lessonlearn_Project, A31TestProjectLL, A31files, A31Imgs

admin.site.register(A31Imgs)
admin.site.register(A31files)
...
class lesson_learnAdmin(admin.ModelAdmin):
    fieldsets = (
        (None, {
            'fields': (
            'Category', 'Object', 'Symptom', 'Reproduce_Steps', 'Root_Cause', 'Solution', 'Action', 'Status', 'Photo',
            'video', 'editor', 'edit_time')
        }),
        # ('Advanced options',{
        #     'classes': ('collapse',),
    ...
    list_display = (
    'Category', 'Object', 'Symptom', 'Reproduce_Steps', 'Root_Cause', 'Solution', 'Action', 'editor', 'edit_time')
    # 列表里显示想要显示的字段
    list_per_page = 200
    # 满50条数据就自动分页
    ordering = ('-edit_time',)
    ...
    search_fields = (
    'Category', 'Object', 'Symptom', 'Reproduce_Steps', 'Root_Cause', 'Solution', 'Action', 'Status', 'editor',
    'edit_time')  # 搜索字段
