...
def CapitalExpenditure_Summary(request):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('isGetData') == 'SEARCH':
            ...
            CapitalExpenditure_obj = CapitalExpenditure.objects.filter(**Check_dic_CapitalExpenditure)
            ...
            update_dic = {
                ...
                "Budget_Quantity": Budget_Quantity if Budget_Quantity and Budget_Quantity != "null" else None,
                "Estimated_Original_Currency": Estimated_Original_Currency,
                "Estimated_Original_Price": Estimated_Original_Price,
                "Equivalent_To_RMB": Equivalent_To_RMB if Equivalent_To_RMB and Equivalent_To_RMB != "null" else None,
                "Payment_Terms": Payment_Terms,
                "Depreciation_Months": Depreciation_Months if Depreciation_Months and Depreciation_Months != "null" else None,
                ...
                "Annual_Increase_PerYear": Annual_Increase_PerYear if Annual_Increase_PerYear and Annual_Increase_PerYear != "null" else None,
                "Investment_Benefits_PerYear": Investment_Benefits_PerYear if Investment_Benefits_PerYear and Investment_Benefits_PerYear != "null" else None,
                "Cash_Inflows_PerYear": Cash_Inflows_PerYear if Cash_Inflows_PerYear and Cash_Inflows_PerYear != "null" else None,
                "Payback_Period": Payback_Period if Payback_Period and Payback_Period != "null" else None,
                "Subscription_Status": Subscription_Status,
                "Subscription_Quantity": Subscription_Quantity if Subscription_Quantity and Subscription_Quantity != "null" else None,
                "Subscription_Amount": Subscription_Amount if Subscription_Amount and Subscription_Amount != "null" else None,
                "Entry_Amount": Entry_Amount if Entry_Amount and Entry_Amount != "null" else None,
            }
            # print(update_dic)
            try:
            ...
            #     Check_dic_CapitalExpenditure["PlanYear"] = YearSearch_backup
            CapitalExpenditure_obj = CapitalExpenditure.objects.filter(**Check_dic_CapitalExpenditure)
        elif request.POST.get('action') == 'addBudgetExternal':
            # ID = request.POST.get('ID')
            PlanYear = request.POST.get('PlanYear')
            Customer = request.POST.get('Customer')
            BudgetCode = request.POST.get('BudgetCode')
            ...
            add_dic = {
                ...
                "Subscription_Amount": Subscription_Amount if Subscription_Amount and Subscription_Amount != "null" else None,
                "Entry_Amount": Entry_Amount if Entry_Amount and Entry_Amount != "null" else None,
                "creator": onlineuser,#只有受哦那个新增需要记录
            }
            print(add_dic, 'add_dic')
            try:
                with transaction.atomic():
                    CapitalExpenditure.objects.create(**add_dic)
            except Exception as e:
                # alert = '此数据正被其他使用者编辑中...'
                alert = str(e)
                print(alert)
        ...
        if CapitalExpenditure_obj:
            for i in CapitalExpenditure_obj:
                # print(i)
                mock_data.append(
                    {
                        "id": i.id, "PlanYear": i.PlanYear, "Customer": i.Customer, "BudgetCode": i.BudgetCode,
        ...
        if DepartmentSeven and YearSearch:
            # print(Check_dic_CapitalExpenditure, YearSearch, DepartmentSeven)
            if Customer == "C38":
                if C38CustomerT88AIODepartmentCode.objects.filter(Year=YearSearch) and DepartmentSeven:
                    ...
                    if CapitalExpenditure.objects.filter(PlanYear=YearSearch):
                        ...
                        tables_dic2 = {
                            "data": [
                                {"C38_T89": "數量",
                                 "Annual_Budget": CapitalExpenditure.objects.filter(PlanYear=YearSearch,
                                                                                    Application_Department__contains=DepartmentSeven).exclude(
                                     Application_Department=AIOT88_DeparmentCode).filter(BudgetCode="預算外").aggregate(
                                     total=Sum('Subscription_Quantity'))[
                                     'total'] if CapitalExpenditure.objects.filter(PlanYear=YearSearch,
                                                                                   Application_Department__contains=DepartmentSeven).exclude(
                                     Application_Department=AIOT88_DeparmentCode).filter(BudgetCode="預算外").aggregate(
                                 ...
                                 "Unsubscribed":
                                     CapitalExpenditure.objects.filter(PlanYear=YearSearch,
                                                                       Application_Department__contains=DepartmentSeven,
                                                                       Subscription_Status="未申購").exclude(
                                         Application_Department=AIOT88_DeparmentCode).filter(
                                         BudgetCode="預算外").aggregate(
                                         total=Sum('Budget_Quantity'))[
                                         'total'] if CapitalExpenditure.objects.filter(PlanYear=YearSearch,
                                                                                       Application_Department__contains=DepartmentSeven,
                        ...
                        tables.append(tables_dic3)
                        tables.append(tables_dic4)
                else:
