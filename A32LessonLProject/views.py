        ...
        if request.POST.get("isGetData") == "submit":
            ...
            try:
                ...
                if editID:
                    ...
                    # 获取图片对象
                    # images = A32Imgs.objects.filter(single__in=image_ids)
                    images = A32Imgs.objects.filter(id__in=photos_to_delete)#最好是用id，这样搜索时就需要返回带id的信息
                    # print(images)
                    ...
                    if videolist:
                        for f in videolist:
                            ...
                            empt.save()
                            editlesson.video.add(empt)
                    vedios = A32files.objects.filter(id__in=videos_to_delete)  # 最好是用id，这样搜索时就需要返回带id的信息
                    # print(vedios)
